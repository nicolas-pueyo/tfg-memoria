...
\noindent \textbf{Palabras clave:} Inteligencia Artificial, \textit{On-Premise}, RAG, OpenCTI, ChromaDB, OpenWebUI, Indicadores de Compromiso, Ciberseguridad.
\
\newpage
...
\vspace{1.5em}
	...
	\noindent \textbf{Keywords:} Artificial Intelligence, On-Premise, RAG, OpenCTI, ChromaDB, OpenWebUI, Indicators of Compromise, Cybersecurity.
...
\begin{itemize}
    \item \textbf{URLs} identificadas como maliciosas, habitualmente vinculadas a servidores de descarga de cargas útiles, portales de suplantación de identidad o paneles de control de botnets.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textit{CyberOSS-2.0-20B (CyberPal-2.0-20B):} Modelo avanzado de la \textit{suite} de seguridad de código abierto CyberPal 2.0, ajustado sobre la arquitectura gpt-oss-20b mediante el \textit{pipeline} de enriquecimiento y trazado de razonamiento técnico \textit{SecKnowledge 2.0}~\cite{levi2025toward}. En su cuantización de 4 bits (Q4\_K\_M), el tamaño del archivo asciende a 15,8 GB, demandando un mayor esfuerzo de lectura por parte de la memoria RAM del servidor, pero aportando un análisis conceptual de nivel experto.
    \end{itemize}

    La elección de esta pareja de modelos permite someter la herramienta a una validación práctica frente a los benchmarks de referencia de la literatura científica~\cite{levi2025toward}.
...
\begin{itemize}
    \item \textbf{Cliente de consultas semánticas:} Componente que se ejecuta en tiempo real dentro del flujo de la conversación. Permite realizar búsquedas exactas e intermedias sobre observables e indicadores bajo la estructura del estándar STIX 2.1, rastreando de forma automática las relaciones lógicas creadas por los analistas y localizando reportes técnicos relevantes que sirvan como evidencia contextualizada para el modelo.
    \item \textbf{Ingestor de entidades:} Módulo encargado del aprovisionamiento masivo de datos históricos. Implementa el protocolo de paginación oficial basado en cursores de OpenCTI para descargar de forma secuencial y ordenada colecciones completas de actores de amenazas, malware, tácticas, reportes e indicadores técnicos, almacenando la información de ciberinteligencia en bruto localmente en formato JSON.
    \item \textbf{Normalizador semántico e indexador:} extrae las relaciones complejas y las unifica en plantillas textuales estructuradas y metadatos legibles para la IA. Posteriormente, un \textit{script} de vectorización automatizado codifica estos textos normalizados en representaciones densas de 384 dimensiones empleando el modelo local \texttt{all-MiniLM-L6-v2}, insertándolos finalmente mediante un algoritmo por lotes en ChromaDB bajo una métrica de similitud de distancia del coseno para dar soporte a las búsquedas de proximidad semántica.
\end{itemize}
\end{itemize}
...
Una vez generados los \textit{embeddings}, el siguiente paso consiste en cargarlos en ChromaDB. Para ello, el \textit{script} de inserción se conecta al servidor de ChromaDB. Para cada tipo de entidad, el \textit{script} crea o recupera una colección con el mismo nombre de la entidad. Las colecciones se configuran utilizando similitud coseno.

Antes de insertar los documentos en ChromaDB, el \textit{script} valida que los textos normalizados, los metadatos y los embeddings tengan la misma longitud. Esta validación evita desalineaciones entre documentos, vectores y metadatos. Posteriormente, la inserción se realiza por lotes, asociando a cada documento su identificador, su texto, su \textit{embedding} y sus metadatos. De este modo, cada entrada almacenada en ChromaDB conserva tanto la representación vectorial necesaria para la búsqueda semántica como la información textual y de trazabilidad necesaria para interpretar el resultado recuperado.
...
Cuando el \textit{backend} ya ha obtenido la información explícita de OpenCTI y el contexto semántico de ChromaDB, se unen ambos bloques en un único contexto final. Este paso aparece en la Figura~\ref{fig:secuencia} como la fase de construcción de contexto.
...
Con el contexto ya recuperado, la \textit{pipeline} construye el \textit{prompt} final que se envia al modelo. En este punto comienza la fase de generación de respuesta, representada en la parte inferior de la Figura~\ref{fig:secuencia}. La \textit{pipeline} crea el \textit{prompt} combinando el contexto recuperado con la pregunta del usuario y unas instrucciones concretas de análisis para el modelo, como la enumeración de objetos del contexto o que comente la separación entre contexto fuerte y contexto inferido.

La llamada al modelo local corresponde a la comunicación $6$ de la Figura~\ref{fig:despliegue}. La \textit{pipeline} envía el \textit{prompt} a Ollama, que ejecuta el modelo seleccionado. Existe un matiz en la implementación que es transparente al usuario pero relevante en el desarrollo del proyecto: \textit{CyberOSS-2.0-20B} utiliza una plantilla de chat diferente al estándar que espera \textit{Ollama}, empleando \textit{tokens} específicos como \texttt{<|start|>user}, \texttt{<|end|>} y \texttt{<|start|>assistant} para indicar cuándo termina la pregunta del usuario y empieza la respuesta. Estos mensajes tienen que ser incluidos en el mensaje que se envía a \textit{Ollama}, y es relevante, por ejemplo, si en un futuro se quiere añadir un nuevo modelo, dado que habría que revisar qué plantilla de chat utiliza.
...
\begin{figure}[] 
	...
	\caption{Flujo completo de consulta}
	\label{fig:secuencia}
\end{figure}
...
Para evaluar la viabilidad operativa de los modelos Lily y CyberOSS\footnote{Para este capítulo se utilizarán estos nombres acortados para referirse a los modelos debido a la cantidad de menciones a los mismos.} en el flujo de trabajo de un analista de ciberseguridad, se diseñó una batería de 7 consultas específicas basadas en indicadores reales inyectados en la base de datos vectorial mediante OpenCTI (Tabla~\ref{tab:preguntas-evaluacion}).
...
\begin{table}[H]
	...
	\begin{tabular}{ l c c }
		\toprule
		\textbf{Pregunta} & \textbf{Lily} & \textbf{CyberOSS}  \\
		\midrule
		...
		\bottomrule
	\end{tabular}
	\caption{Comparativa de tiempos de respuesta (en formato \textit{mm:ss,cs})}
	\label{tab:tiempos}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabular}{ l c c }
		\toprule
		\textbf{Criterio} & \textbf{Lily} & \textbf{CyberOSS}  \\
		\midrule
...
\begin{table}[h!]
	...
	\begin{tabular}{lr}
		...
		\midrule
		\multicolumn{1}{r}{\textbf{Total}} & \textbf{305} \\
		\bottomrule
	\end{tabular}
